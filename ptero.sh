...
function update_panel() {
    ...
    cd "$PANEL_DIR" || return 1
    php artisan down --message="Sedang update, tunggu sebentar." || true
    curl -fsSL https://github.com/pterodactyl/panel/releases/latest/download/panel.tar.gz \
        -o /tmp/panel.tar.gz
    tar -xzf /tmp/panel.tar.gz -C "$PANEL_DIR"
    rm -f /tmp/panel.tar.gz
...
function restore_system() {
    ...
    if [ "$RMODE" = "1" ] || [ "$RMODE" = "2" ]; then
        ...
        mysql_secure "$DB_USER" "$DB_PASS" -h 127.0.0.1 "$DB_NAME" < "$BACKUP_PATH/panel_db.sql" \
            || { fail "Restore database gagal."; release_lock; trap - EXIT; return 1; }
    fi
    ...
    if [ "$RMODE" = "1" ] || [ "$RMODE" = "5" ]; then
        ...
        [ -f "$BACKUP_PATH/server_volumes.tar.gz" ] && tar -xzf "$BACKUP_PATH/server_volumes.tar.gz" -C /var/lib
    fi
...
function schedule_auto_backup() {
    ...
    if ! echo "$BACKUP_TIME" | grep -Eq '^[0-2][0-9]:[0-5][0-9]$'; then
        fail "Format jam tidak valid. Gunakan HH:MM, contoh 03:00."
        pause
        return 1
    fi
...
function view_logs_menu() {
    ...
    while true; do
        ...
        case "$LOG_OPT" in
            1) journalctl -fu wings --no-pager 2>/dev/null || tail -f /var/log/syslog ;;
            2) journalctl -fu pteroq --no-pager 2>/dev/null ;;
...
function bulk_server_action() {
    ...
    case "$BO" in
        ...
        3)
            ...
            mapfile -t containers < <(docker ps --filter "name=^[a-f0-9-]{36}$" -q 2>/dev/null)
...
function reset_node_network() {
    ...
    systemctl start wings 2>/dev/null || true
    echo -e "${GREEN}Network node direset.${NC}"
    pause
}
...
function self_check() {
    ...
    for fn in install_all install_panel_only install_wings_only_full \
              ...
              load_config save_config notify_detail; do
